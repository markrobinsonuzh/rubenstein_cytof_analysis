# 
# knitr::spin("cytofWorkflow_Cancer_20clusts_merge3_PDFs_panels.R")
...
plot_clustering_heatmap_wrapper <- function(expr, expr01, cluster_rows = TRUE,
  ...
  legend_breaks = round(seq(from = 0, to = max(expr01_median[,-1]), length.out = 6), 2)
...
#                                 expr01 = expr[, lineage_markers_ord], 
#                                 cell_clustering = cell_clustering1, color_clusters = color_clusters)


library(ComplexHeatmap)

plot_clustering_heatmap_wrapper2 <- function(expr, expr01, cluster_rows = TRUE,
...
cell_clustering1m <- factor(as.character(cell_clustering1m),
                            levels = levels(cell_clustering1m)[c(5,7,8,10,1,4,2,3,6,9)])
...
ggsave(filename = "tSNE_panel_A.pdf", plot = ggp, width = 6, height = 4)
...
carsten_o <- c("CD16","CD94","CD33","CD14","FoxP3","TCRgd","CD19","CD8","CD4")
carsten_o <- c(carsten_o, setdiff(lineage_markers_ord, carsten_o))

pchw <- plot_clustering_heatmap_wrapper(expr = expr[, carsten_o], cluster_rows = FALSE,
                                        expr01 = expr[, carsten_o], cell_clustering = cell_clustering1m,
                                        color_clusters = color_clusters)
ggsave(filename = "heatmap_panel_A.pdf", pchw$gtable, width = 10, height = 4)



cell_counts <- table(cell_clustering1m, sample_ids)
props <- t(t(cell_counts) / colSums(cell_counts))


ph <- pheatmap(props, color = color, display_numbers = TRUE, scale = "row",
...
ph <- pheatmap(props, color = color, display_numbers = TRUE,
               number_color = "black", fontsize_number = 5, annotation_col = annotation_col, 
               annotation_colors = annotation_colors, cluster_rows = FALSE, cluster_cols = FALSE)

ph <- pheatmap(sqrt(props), color = color, display_numbers = FALSE,
               number_color = "black", fontsize_number = 5, annotation_col = annotation_col, 
               annotation_colors = annotation_colors, cluster_rows = FALSE, cluster_cols = FALSE)

ggsave(filename = "cluster_freq_sqrt_C.pdf", ph$gtable, width=6, height=6)


props1 <- cbind(average_healthy=rowMeans(props[,1:4]), as.matrix(props))
ph <- pheatmap(sqrt(props1), color = color, display_numbers = FALSE,
               number_color = "black", fontsize_number = 5, annotation_col = annotation_col, 
               annotation_colors = annotation_colors, cluster_rows = FALSE, cluster_cols = FALSE)

ggsave(filename = "cluster_freq_sqrt_w_average_C.pdf", ph$gtable, width=6, height=6)
...
#                                  sample_ids = sample_ids, cell_clustering = cell_clustering1m, 
#                                  color_clusters = color_clusters, cluster_merging = NULL)

## ----plot-clustering-heatmap2-1-unnormalized, fig.height = 12
pdf("heatmap_lineage+CD38_original_C.pdf", width = 10, height = 5)
pchw2 <- plot_clustering_heatmap_wrapper2(expr = expr, expr01 = expr, cluster_rows = FALSE,
                                 lineage_markers = lineage_markers, functional_markers = "CD38", 
                                 sample_ids = sample_ids, cell_clustering = cell_clustering1m, plot_cluster_annotation = FALSE,
                                 color_clusters = color_clusters, cluster_merging = NULL)
dev.off()
pchw2
...
pdf("heatmap_lineage+GrzB_original_C.pdf", width=10, height=5)
pchw2 <- plot_clustering_heatmap_wrapper2(expr = expr, expr01 = expr, cluster_rows = FALSE,
                                 lineage_markers = lineage_markers, functional_markers = "Grz_B", 
                                 sample_ids = sample_ids, cell_clustering = cell_clustering1m, plot_cluster_annotation = FALSE,
                                 color_clusters = color_clusters, cluster_merging = NULL)
...
#                                  sample_ids = sample_ids1, cell_clustering = cell_clustering1m, plot_cluster_annotation = FALSE,
#                                  color_clusters = color_clusters, cluster_merging = NULL)

## ----plot-clustering-heatmap2-3-unnormalized, fig.height = 12
pdf("heatmap_lineage+ki67_original_C.pdf", width=10, height=5)
...
dr1 <- dr[dr$patient %in% c("healthy","MUSC10-17"),]
...
## Facet per patient/time
ggp + facet_wrap(patient ~ condition_time, nrow=1) 

ggsave(filename = "tSNE_split_ki67_B.pdf", 
       ggp + facet_wrap(patient ~ condition_time, nrow=1),
       width=12, height=4)
...
ggsave(filename = "tSNE_split_GrzB_B.pdf", 
       ggp + facet_wrap(patient ~ condition_time, nrow=1),
       width=12, height=4)
...
ggsave(filename = "tSNE_split_CD38_B.pdf", 
       ggp + facet_wrap(patient ~ condition_time, nrow=1),
       width=12, height=4)
...
#   scale_color_gradientn(colours = colorRampPalette(rev(brewer.pal(n = 11, name = "Spectral")))(50))
# 
# ## ----tsne-plot-facet-patient-time-4, fig.height = 12----
# ## Facet per patient/time
# ggp + facet_wrap(patient ~ condition_time) 
# 
# 
